...
```{r webgl=TRUE}
white_wine_subset$pH_bucket <- cut(white_wine_subset$pH, breaks = 4,  labels = FALSE, ordered_result = TRUE)
white_wine_subset$alcohol_bucket <- cut(white_wine_subset$alcohol, breaks = 5,  labels = FALSE, ordered_result = TRUE)
group_data <- group_by(white_wine_subset, ordered_category, pH_bucket, alcohol_bucket)
summarised_data <- summarise(group_data, n = n())
...
for (ph_level in ph_interval)
  for (alcohol_level in alcohol_interval){
    normal_level_count <- subset(summarised_data, summarised_data$pH_bucket == ph_level & summarised_data$alcohol_bucket == alcohol_level  & summarised_data$ordered_category == 'normal')
    high_level_count <- subset(summarised_data, summarised_data$pH_bucket == ph_level & summarised_data$alcohol_bucket == alcohol_level  & summarised_data$ordered_category == 'high')
    if (nrow(normal_level_count) > 0){
      normal_level_count <- normal_level_count$n
    }else{
      normal_level_count <- 1
...
ggplot(data = white_wine, aes(x = factor(1), fill = ordered_category)) +
  ...
  xlab('') +
  ylab('') +
  scale_fill_manual('Categories of Wine Quality', values = cols, breaks = c("low", "normal", "high"), labels = c("Low Quality (Grade 1, 2, 3 and 4)", "Normal Quality (Grade 5, 6 and 7)", "High Quality(Grade 8 and 9)")) +
...
p_density = ggplot(data = white_wine, aes(alcohol, ..density..)) + 
  ...
  ylab('Density') + 
  scale_colour_manual('Categories of Wine Quality', values = cols, breaks = c("low", "normal", "high"), labels = c("Low Quality (Grade 1, 2, 3 and 4)", "Normal Quality (Grade 5, 6 and 7)", "High Quality(Grade 8 and 9)")) + 
...
ggplot(pH_ratio, aes(x = variale_intervel)) + 
  geom_smooth(aes(y = high_ratio, colour = "High Quality to Normal"), se = FALSE) + 
  geom_smooth(aes(y = low_ratio, colour = "Low Quality to Normal"), se = FALSE) +
  ...
  scale_colour_manual("Ratios of Counts", 
                      values = c("High Quality to Normal" = "red", "Low Quality to Normal" = "blue")) + 
  blank_theme
```
